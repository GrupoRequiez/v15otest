...
class ProductLabelMrp(models.AbstractModel):
    ...
    def get_data(self, mrp_id):
        ...
        data = {}
        if result:
            prod_tmpl_id = self.env['product.template'].search([('default_code', '=', result[3])])
            trans_value = result[4]
            if prod_tmpl_id:
                trans_id = self.env['ir.translation'].search(
                    [('res_id', '=', prod_tmpl_id.id)], limit=1)
                if trans_id:
                    trans_value = trans_id.value
            data.update({'date': result[0], 'type': result[1],
                         'partner_name': result[2][:41],
                         'product_default': result[3],
                         'product_name': trans_value,
